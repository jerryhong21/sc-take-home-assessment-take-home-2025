...
func GenerateData() []Folder {
	// generates rng names from codename library, error is ignored
	rng, _ := codename.DefaultRNG()

	// definition and initialisation of empty folder slice (vector)
	tree := []Folder{}

	for i := 0; i < MaxRootSet; i++ {
		orgId := uuid.FromStringOrNil(DefaultOrgID)
		// every 3 iterations generates new v4 of uuid and replaces default orgId
		// ensures some folders have unique ordIgs instead of default ones
		if i%3 == 0 {
		...
		// create channel - channels are conduits which goroutines use to
		// receive and send data
		subtree := make(chan []Folder)

		// concurrently generates a subtree
		go func() {
			// send result of generateTree to subtree channel
		...
		// <- receives the message from subtree
		// appends the subtree to the tree slice
		// ... unpacks the slice
...
// recursively generates folder tree
func generateTree(depth int, tree []Folder) []Folder {
	...
	// a random number of child folders is created for each folder
